...
import { FindTripRequestByIdController } from "#/features/trip/infrastructure/controller/find-by-id-controller.ts";
import { FindTripRequestByIdMapper } from "#/features/trip/infrastructure/presentation/mapper/find-by-id-mapper.ts";
...
describe("FindTripRequestByIdController", () => {
    ...
    let useCase: FindTripByIdUseCase;
    let controller: FindTripRequestByIdController;
    ...
    beforeEach(() => {
        ...
        useCase = {
            execute: vi.fn()
        } as unknown as FindTripByIdUseCase;

        controller = new FindTripRequestByIdController(useCase);
    });
    ...
    it("should execute use case and send success response", async () => {
        ...
        vi.spyOn(FindTripRequestByIdMapper, "toInput")
            .mockReturnValue({
                tripRequestId: "trip-1"
            });

        vi.spyOn(FindTripRequestByIdMapper, "toResponse")
        ...
        expect(FindTripRequestByIdMapper.toInput)
